...
1. AIの最終レシピに「📖説明:」または「📖説明」がある場合
   → その後の文章を一字一句変更せずにそのまま使用（要約・変更・短縮は絶対禁止）
...
{
  "title": "料理名（【】は除去）",
  "description": "料理の説明（📖説明がある場合は必ずそのまま全文を使用、ない場合のみユーザーのコメントから1-2文で作成）",
  ...
  "tips": "コツやポイント（💡コツがある場合は必ずそのまま全文を使用、ない場合のみユーザーが書いたものやレシピから推測）",
  "calories": 300,
  "salt_g": 1.5,
  "cooking_time_min": 15,
  "tags": ["肉", "主菜", "時短"]
...
- 必ず有効なJSONを出力してください
- 「📖説明」「💡コツ」がある場合は、その内容を一字一句変更せずに抽出すること（これが最優先）
`.trim();
